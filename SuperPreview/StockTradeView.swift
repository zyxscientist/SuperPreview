...
struct StockTradeView: View {
    ...
    @StateObject var priceQuantityViewModel = PriceQuantityViewModel()
        ...
        ScrollView {
            LazyVStack(spacing: 0) {
                ...
                Group {
                    ...
                    // 行情栏
                    SymbolQuoteView(openGraphView: $openGraphView)
                ...
                Group{
                    // 十档报价
                    StockTradeTapeView(openTapeView: $openTapeView)
                        .padding(.top, 5)
                        .overlay {
                            VStack {
                                Spacer()
                    ...
                    // 价格输入栏
                    PriceInputField(priceTargetingMenu: $priceTargetingMenu)
                        .zIndex(2)
                    
                    // 数量输入栏
                    QuantityInputField(quantityQuickType: $quantityQuickType)
                    ...
                    // 金额栏
                    OrderAmountView(advanceSetting: $advanceSetting)
                        .zIndex(1)
                    ...
                    MaximumBuyingPowerView()
                    BuyAndSellTipsView()
                    TradeCompactListView()
                }
...
struct StockTradeViewPreviews: PreviewProvider {
                ...
                VStack(spacing: 8) {
                    ...
                    VStack(spacing: 0) {
                        ...
                        HStack(spacing: 5) {
                            Image("ysl_mini_logo")
                            Text("交易服务由云锋证券提供")
                                .modifier(CustomFontModifier(size: 9, font: .regular))
