...
class PriceQuantityViewModel: ObservableObject {
    ...
    let totalHoldingAssetOrderAmountView: Double = 1000000.00
    
    
    func getOrderAmountView(){
        amount = price * Double(quantity)
        predictivePercentage = amount*100/(totalHoldingAssetOrderAmountView+amount) // 百分比来的
