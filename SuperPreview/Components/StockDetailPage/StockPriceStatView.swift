...
struct StockPriceStatView: View {
    
    let stockStats: [StockStatData]
    @State private var isExpanded = false
    @State private var blurOrderAmountView: CGFloat = 7
    ...
    var body: some View {
                    ...
                    LazyVGrid(columns: columns, alignment: .leading, spacing: 5) {
                        ForEach(Array(stockStats.dropFirst(9)), id: \.id) { stat in
                            StockStatView(data: stat)
                                ...
                                .blur(radius: blurOrderAmountView)
                        }
                    }
                    .padding(.top, 5)
                    .padding(.horizontal, 15)
        ...
        .onTapGesture {
            ...
            withAnimation(.spring(response: 0.4, dampingFraction: 0.8, blendDuration: 0.2)) {
                isExpanded.toggle()
                blurOrderAmountView = isExpanded ? 0 : 5
            }
        }
...
struct StockPriceBlockView: View {
    var body: some View {
...
#Preview {
    StockPriceStatView(stockStats: stockStats)
}
